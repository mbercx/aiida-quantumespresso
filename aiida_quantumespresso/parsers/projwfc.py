...
class ProjwfcParser(Parser):
    ...
    def parse(self, **kwargs):
        ...
        # Parse basic info and warnings, and output them as `output_parameters`
        parsed_out, logs_out = parse_output_base(out_file, 'PROJWFC')
        self.emit_logs(logs_out)
        self.out('output_parameters', Dict(dict=parsed_out))
    ...
    def parse_xml(self):
        """Parse the XML file to obtain the required information for the orbital parsing."""
    ...
    def _parse_bands_and_projections(self, out_info_dict):
        ...
        out_info_dict['orbitals'] = self._find_orbitals_from_statelines(out_info_dict)
        ...
        if spin:
            ...
            bands_data1, projection_data1 = self._spin_dependent_subparser(out_info_dict)
            append_nodes_list += [('projections_up', projection_data1), ('bands_up', bands_data1)]
            out_info_dict['spin_down'] = True
            bands_data2, projection_data2 = self._spin_dependent_subparser(out_info_dict)
            append_nodes_list += [('projections_down', projection_data2), ('bands_down', bands_data2)]
        else:
            out_info_dict['spin_down'] = False
            bands_data, projection_data = self._spin_dependent_subparser(out_info_dict)
            append_nodes_list += [('projections', projection_data), ('bands', bands_data)]

        return append_nodes_list

    @staticmethod
    def _find_orbitals_from_statelines(out_info_dict):
    ...
    @classmethod
    def _spin_dependent_subparser(cls, out_info_dict):
        ...
        pdos_arrays = cls._spin_dependent_pdos_subparser(out_info_dict)
    ...
    @staticmethod
    def _spin_dependent_pdos_subparser(out_info_dict):
        ...
        def natural_sort_key(sort_key, _nsre=re.compile('([0-9]+)')):
            ...
            For example, ``["2", "11", "1"]`` will be sorted to ``["1", "2", "11"]`` instead of ``["1", "11", "2"]``
