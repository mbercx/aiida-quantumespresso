...
from aiida.plugins import OrbitalFactory, DataFactory

from aiida_quantumespresso.parsers import QEOutputParsingError
from aiida_quantumespresso.parsers.parse_raw.base import (
    convert_qe2aiida_structure, convert_qe_to_kpoints, convert_qe_time_to_sec
)
...
ProjectionData = DataFactory('array.projection')
...
class ProjwfcParser(Parser):
    ...
    def parse(self, **kwargs):
        """Parses the data folder, stores results.
        ...
        output_parameters = {
            'code_version': re.search(r'Program\sPROJWFC\sv\.([\.\d]+)\sstarts', header).groups()[0],
            'wall_time_seconds': convert_qe_time_to_sec(re.search(r'CPU(.+)WALL', lowdin_block).groups()[0]),
        }
        ...
        structure = convert_qe2aiida_structure(parsed_xml['structure'])
    ...
    def read_stdout(self) -> Tuple[str, list, str]:
        """Read the standard output.

        The method will make sure that the standard output file is present and that the job has completed, and exit
        with the corresponding exit code if not.

        :return: tuple with three outputs:

            * ``header``: all output up to the first block of k-point outputs.
            * ``kpoint_blocks``: a list of text blocks with energies/projections for each k-point
            * ``lowdin_block``: all output after the k-point blocks, i.e. after "Lowdin Charges:" has been printed
        """
        ...
        # Split the stdout into header, k-point blocks and the (final) Lowdin block
        stdout_blocks = stdout.split('Lowdin Charges:')
        lowdin_block = stdout_blocks[-1]
        stdout_blocks = stdout_blocks[0].split('k = ')
        header = stdout_blocks[0]
        kpoint_blocks = stdout_blocks[1:]

        # The Lowdin block also contains the footer, so check for 'JOB DONE' there to see if the stdout is complete
        if not 'JOB DONE' in lowdin_block:
            return self.exit(self.exit_codes.ERROR_OUTPUT_STDOUT_INCOMPLETE)

        return header, kpoint_blocks, lowdin_block

    def _parse_xml(self) -> Tuple[dict, AttributeDict]:
        """Parse the XML file.
    ...
    def _parse_orbitals(header: str, structure: StructureData, non_collinear: bool, spinorbit: bool) -> list:
        ...
        # TODO: Add test where radial nodes are not zero
        new_orbital_dicts = []
    ...
    def _parse_pdos_files(self, nspin: int, spinorbit: bool) -> Tuple[ArrayLike, ArrayLike, ArrayLike]:
        ...
        if nspin != 4 or spinorbit:
            ...
            if nspin == 2:
                # Reshuffle the order of the columns so the 'up' spin columns are first
                pdos_array = np.concatenate([pdos_array[:, 0::2], pdos_array[:, 1::2]], axis=1)
            if nspin == 4:
                # Reshuffle the order of the columns in accordance with order of orbitals for spin-orbit
            ...
            for wfc_pdos_array in [pdos_atm_array_dict[name] for name in pdos_file_names]:
                # Reshuffle the PDOS columns so the 'up' spin columns _for this l number_ come first
    ...
    @classmethod
    def _build_bands_and_projections(cls, kpoints, bands, energy, orbitals, projections, pdos_array, nspin):
    ...
    @staticmethod
    def _intialize_bands_projection_data(kpoints, bands, energy, orbitals, projections, pdos_array):
        """Initialize an instance of ``BandsData`` and correpsonding ``ProjectionData``."""
